...
public class Article {
    ...
    /*
        @ManyToOne
         - N:1 관계설정
         - Article 엔티티와 User 엔티티 간의 관계와 방향성을 고려해서 연관관계 설정
    ...
    */
    ...
    /*
        @OneToMany
         - 1:N 관계설정, 일반적으로 양방향 관계설정, 참조타입이 List
         - Article 엔테테와 File 엔티티 간의 관계와 방향성을 고려해서 연관관계 설정
        ...
        @Transactional
         ...
         - 하나의 SELECT는 한 번의 세션처리로 트랜젝션으로 처리하지 않으면 no session 에러 발생
         - 트랜젝션으로 처리하기 위해서 처리 메서드에 @Transactional 선언

        @ToString(exclude="제외할 속성")
         - 엔티티 간 양방향 관계설정에서 toString()을 호출할 경우 무한순환 호출이 실행
         - 무한순환 호출이 발생하면 StackOverflow 에러 발생
         - 양방향으로 관계설정된 엔티티에서 한 쪽을 ToString()에서 제외
    */
