...
public class SecurityConfig {
    ...
    /*
        ...
        인가 설정
         - MyUserDetails의 사용자 권한 목록을 제공하는 getAuthorities()에서 반드시 접두어 ROLE_ 붙여야 함
         - ROLE_ 접두어를 안 붙이면 hasAuthority(), hasAnyAuthority()로 권한 설정
         - 존재하지 않는 요청 주소에 대해서 시큐리티는 로그인 페이지로 redirect 수행하기 때문에 마지막에 anyRequest().permitAll() 권한 설정
     */
    ...
    public SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        // 기타 보안 설정
        http.csrf(configure -> configure.disable()); // 해커가 인증을 유도하여 정보를 탈취하는 방식(지금은 실습을 위해 잠시 off)
